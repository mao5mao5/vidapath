...
export function isCluster(feature: any): boolean | undefined {
...
export function createStrokeStyle(color: string, opacity: number = 0.5): Style {
...
export function createLineStrokeStyle(color: string, opacity: number = 0.5): Style {
  let colorArray = hexToRgb(color);
  let colorWithOpacity = colorArray.slice();
  colorWithOpacity[3] = opacity;
  ...
  return new Style({
    stroke,
    image: new Circle({radius: 5, stroke})
  });
}

function createStroke(opacity: number): Stroke {
  return new Stroke({
    color: colorWithOpacity([0, 0, 0, 1], opacity),
    width: 2
  });
}
...
export function createFillStyle(color: string, opacity: number = 0.5): Style {
  let colorArray = hexToRgb(color);
  let colorWithOpacity = colorArray.slice();
  colorWithOpacity[3] = opacity;
  ...
  (circleStyle as any).setOpacity(opacity);
...
export function createColorLineStyle(color: string, opacity: number = 0.5): Style {
  let colorArray = hexToRgb(color);
  let colorWithOpacity = colorArray.slice();
  colorWithOpacity[3] = opacity;
...
export function createTextStyle(text: string, fontSize: string = '22px', fill: Fill = textFill, stroke: Stroke = textStroke): Style {
  return new Style({
    text: new Text({
      text,
...
export function changeOpacity(style: Style, opacity: number): void {
  ...
  if (image) {
    (image as any).setOpacity(opacity);
  }
}
...
function createDefaultColor(name: string, hexaCode: string): DefaultColor {
  return {
    name,
...
export const defaultColors: readonly DefaultColor[] = Object.freeze([
